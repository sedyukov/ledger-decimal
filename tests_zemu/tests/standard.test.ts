...
describe('Standard', function () {
  ...
  test.each(DEVICE_MODELS)('get address', async function (m) {
    ...
    try {
      ...
      const resp = await app.getAddressAndPubKey(path, 'dx')
      ...
      expect(resp.bech32_address).toEqual('dx1z4x4r0ljrkserqrqpn65g98h50mjkc0qxas0ps')
  ...
  test.each(DEVICE_MODELS)('show address', async function (m) {
    ...
    try {
      ...
      const respRequest = app.showAddressAndPubKey(path, 'dx')
      ...
      expect(resp.bech32_address).toEqual('dx1zeqn8fd747nftfaszvr3p253nd0qggk9g4ln6z')
      expect(resp.compressed_pk.length).toEqual(33)
    } finally {
      await sim.close()
    }
  ...
  test.each(DEVICE_MODELS)('show address HUGE', async function (m) {
    ...
    try {
      ...
      const resp = await app.showAddressAndPubKey(path, 'dx')
      console.log(resp)
  ...
  test.each(DEVICE_MODELS)('show address HUGE Expect', async function (m) {
    ...
    try {
      ...
      expect(resp.bech32_address).toEqual('dx1fmn4tqa8ats32h9647gclhrcdfsakj7lz48kxh')
      expect(resp.compressed_pk.length).toEqual(33)
    } finally {
      await sim.close()
    }
  ...
  test.each(DEVICE_MODELS)('sign basic normal', async function (m) {
    ...
    try {
      ...
      const respPk = await app.getAddressAndPubKey(path, 'dx')
